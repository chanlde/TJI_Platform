...
| `MqttTopicLayout` / `mqttTopicsFor(ProductType)` | 各产品主题前缀：`FireBucket/devices/{sn}/…`、`SolarClean/devices/{sn}/…`。 |
| `MqttSubscriptionManager` | `subscribeToDevices(snList, productType)`：对本批 SN 订阅 lifecycle/status；并记录 SN→产品线，供入站分发与退订。 |
...
假设新产品名为 `Acme`，并已有 MQTT 前缀约定 `Acme/devices/{sn}/lifecycle|status|control`。
...
12. **`MqttSubscriptionManager`**：`ProductType.values()` 已包含新产品时，未知 SN 退订时已会遍历所有产品线主题（已实现多前缀退订）。  
...
| 光伏清洗协议落地 | `Doc/solarCleanProtocol.md` |
