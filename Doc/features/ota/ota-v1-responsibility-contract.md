...
- 状态：draft
- 说明：OTA 第一版职责边界和协议草案，落地前仍需要 App、服务器和 MCU 联调确认。
...
App 已经按统一多产品 MQTT 结构实现，OTA 第一版不再使用旧的 `device/{deviceId}/...` topic。  
光伏清洗 OTA 统一复用 SolarClean 产品线三段 topic：

| 用途 | Topic | 方向 | Retain |
|---|---|---|---|
| 设备信息上报 | `SolarClean/devices/{deviceId}/status` | 单片机 -> App | true |
| App 下发命令 | `SolarClean/devices/{deviceId}/control` | App -> 单片机 | false |
| OTA 状态上报 | `SolarClean/devices/{deviceId}/status` | 单片机 -> App | false |
...
2. App 进入设备页面
3. App 订阅 SolarClean/devices/{deviceId}/status
...
```text
1. 用户点击升级
2. App 发布 START_OTA 到 SolarClean/devices/{deviceId}/control
3. 单片机开始下载固件
...
- 订阅 `SolarClean/devices/{deviceId}/status`
...
- 上电发布设备信息到 `SolarClean/devices/{deviceId}/status`
- `info` 设置 retain
