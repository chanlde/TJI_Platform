...
发布版 SHA1 应来自正式 release keystore 的签名指纹；不要把本机 keystore 路径写入仓库文档。当前项目仍未配置 release signingConfig，后续发版需要把 release keystore 通过安全配置接入 Gradle。
...
- [protocol.md](protocol.md)：无线电检测 MQTT 和 RID payload 接入原则。
- [mcu.md](mcu.md)：设备侧上报、坐标和安全联调要求。
- [server.md](server.md)：高德 Key、绑定字段、Broker 和后端待办。
- [app.md](app.md)：App 代码落点、地图、UI 和测试。
